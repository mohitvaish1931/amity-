...
| `src/constitution/report.ts` | `renderConstitutionReport(input)`: the printable report (executive summary, constitution table, law details, remediation guidance per category, limitations), escaped by `html```; `REMEDIATION` and `REPORT_LIMITATIONS`. |
...
| `src/ui/persist.ts` | `createWorkspaceStore(getStorage)`: opt-in save/restore of Step 1 inputs (spec, configuration, sandbox URL, sensitivity overrides) with a versioned, validated format, a size cap, and results instead of exceptions when storage is missing, blocked or full. Copies only declared fields (never credentials). |
...
- Step 1 page load: `app.js` 197.5 KiB + `app.css` 26 KiB. The graph chunk (React + React Flow, 413 KiB) loads on BUILD. Before code splitting the page loaded 589 KiB of JS up front. Step 2: `app.js` 49 KiB + `app.css` 7 KiB.
